            ...
            return employees.map(emp => {
                ...
                const khususDetails = khususItems.map(k => {
                    ...
                    if (actual === 0 && nominalFlag) {
                        ...
                        else if (searchKey.includes('npl')) searchKey = 'npl';

                        const matchingLogs = empLogs.filter(l => (l.activityName || '').toLowerCase().includes(searchKey));
    ...
    static async syncRealisasiFromWla(employeeId: string, period: string) {
        try {
            ...
            for (const kpi of kpis) {
                ...
                const kpiNameLower = (kpi.kpiName || '').toLowerCase();
                const isPemasaranDana = kpiNameLower.includes('pemasaran dana');
                const isPemasaranKredit = kpiNameLower.includes('pemasaran kredit');
                const isNpl = kpiNameLower.includes('npl');

                const isNominal = isPemasaranDana || isPemasaranKredit || isNpl || (kpi.targetUnit || '').toLowerCase().includes('rp');
                const sumColumn = isNominal ? 'l.nominal_rupiah' : 'l.frekuensi';

                let activityCondition = 'l.id_activity_library = ?';
                let queryParams: any[] = [employeeId, activityId, startDate, endDate];

                if (isPemasaranDana) {
                    activityCondition = `l.id_activity_library IN (SELECT id FROM activity_library WHERE LOWER(activityName) LIKE '%pemasaran dana%')`;
                    queryParams = [employeeId, startDate, endDate];
                } else if (isPemasaranKredit) {
                    activityCondition = `l.id_activity_library IN (SELECT id FROM activity_library WHERE LOWER(activityName) LIKE '%pemasaran kredit%')`;
                    queryParams = [employeeId, startDate, endDate];
                } else if (isNpl) {
                    activityCondition = `l.id_activity_library IN (SELECT id FROM activity_library WHERE LOWER(activityName) LIKE '%npl%')`;
                    queryParams = [employeeId, startDate, endDate];
                }

...
exports.NotifikasiRepository = {
    ...
    async create(notificationData) {
        ...
        const result = await db.run(`INSERT INTO notifications (employee_id, message, type, is_read, created_at, scheduled_for, delivery_channel, related_entity, related_entity_id) 
       VALUES (?, ?, ?, ?, datetime('now'), ?, ?, ?, ?)`, employee_id, message, type, 0, scheduled_for, delivery_channel, related_entity, related_entity_id);
        return {
            id: result.lastID,
            employee_id,
            message,
    ...
    async findScheduledNotifications() {
        const db = await (0, db_1.openDb)();
        const rows = await db.all('SELECT id, employee_id, message, type, is_read, created_at, scheduled_for, delivery_channel, related_entity, related_entity_id FROM notifications WHERE scheduled_for IS NOT NULL AND scheduled_for <= datetime("now") AND is_read = 0');
        return parseJsonFields(rows);
    }
};

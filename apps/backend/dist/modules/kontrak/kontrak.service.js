...
class KontrakService {
    ...
    static async createContract(contractData) {
        try {
            const newContract = await kontrak_repository_1.KontrakRepository.create(contractData);
            if (contractData.addRiwayatJabatan && contractData.riwayatJabatan && newContract.employeeId) {
                try {
                    await riwayatJabatan_repository_1.RiwayatJabatanRepository.create(newContract.employeeId, contractData.riwayatJabatan);
                }
                catch (riwayatError) {

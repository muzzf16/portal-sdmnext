...
class AuthPenggunaController {
    static async login(req, res, next) {
        try {
            ...
            const { token, user } = await auth_pengguna_service_1.default.login(email, password);
            res.status(200).json({
                token,
                user
            });
    ...
    static async register(req, res, next) {
        try {
            ...
            const result = await auth_pengguna_service_1.default.register(name, email, password);

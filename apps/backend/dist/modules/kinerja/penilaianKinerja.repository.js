...
exports.PenilaianKinerjaRepository = {
    ...
    async create(data) {
        const db = await (0, db_1.openDb)();
        const totalWeight = data.kpis.reduce((sum, kpi) => sum + kpi.weight, 0) || 1;
        const weightedScore = data.kpis.reduce((sum, kpi) => sum + (kpi.score * kpi.weight), 0);
        ...
        await db.run('INSERT INTO penilaian_kinerja (id, employeeId, employeeName, period, reviewerName, reviewDate, overallScore, status, strengths, areasForImprovement, employeeFeedback, kpis) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)', Object.values(reviewData));

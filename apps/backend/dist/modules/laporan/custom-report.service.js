...
class CustomReportService {
    static async generateCustomReport(filters, fields, reportType) {
        try {
            if (!reportType) {
                throw new errors_1.AppError('Report type is required', 400);
            }
            if (!Array.isArray(fields) || fields.length === 0) {
                throw new errors_1.AppError('At least one field must be selected', 400);
            }
            let reportData = [];
            switch (reportType) {
                case 'pegawai':
                ...
                case 'kinerja':
                    ...
                    break;
                default:
                    throw new errors_1.AppError(`Unsupported report type: ${reportType}`, 400);
            }
            return reportData;
        }
        catch (error) {
            throw new errors_1.AppError(`Error generating custom report: ${error.message}`, 500);
        }
    }
    static async generateCustomEmployeeReport(filters, fields) {
        const db = await laporan_repository_1.LaporanRepository['generateLaporanPegawai']();
    ...
    static async generateCustomAttendanceReport(filters, fields) {
        const db = await laporan_repository_1.LaporanRepository['generateLaporanPegawai']();
        const dbInstance = db.constructor;
        let selectFields = fields.join(', ');
    ...
    static async generateCustomPayrollReport(filters, fields) {
        const db = await laporan_repository_1.LaporanRepository['generateLaporanPegawai']();
        const dbInstance = db.constructor;
        let selectFields = fields.join(', ');
    ...
    static async generateCustomLeaveReport(filters, fields) {
        const db = await laporan_repository_1.LaporanRepository['generateLaporanPegawai']();
        const dbInstance = db.constructor;
        let selectFields = fields.join(', ');
    ...
    static async generateCustomPerformanceReport(filters, fields) {
        const db = await laporan_repository_1.LaporanRepository['generateLaporanPegawai']();
        const dbInstance = db.constructor;
        let selectFields = fields.join(', ');
...
exports.default = CustomReportService;
//# sourceMappingURL=custom-report.service.js.map
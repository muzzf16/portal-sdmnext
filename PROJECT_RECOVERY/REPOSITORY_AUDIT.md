...
| **SEC-02** | Bypass JWT / Ketiadaan Proteksi Autentikasi Rute | **Kritis (Critical)** | Modul backup, pengguna, pegawai, cuti, absensi, penggajian, kontrak, laporan tidak memiliki proteksi autentikasi pada rute-nya, meski GEMINI.md mengklaim sebaliknya. Server bersifat internet-facing. Keputusan pemilik proyek per 11 Juli 2026: **DITUNDA** hingga sesi development berikutnya. Risiko: akses/modifikasi/pengambilan data tanpa otorisasi dapat terjadi kapan saja selama masa penundaan ini. | Ditunda (Diterima secara sadar) |
...
| **SEC-04** | Privilege Escalation via Self-Registration | **Kritis (Critical)** | Endpoint `POST /api/auth/register` menerima field `role` langsung dari request body client tanpa validasi/whitelist ([auth.pengguna.service.ts:34](file:///opt/portal-sdmv3/apps/backend/src/modules/pengguna/auth.pengguna.service.ts#L34): `role: role?.toLowerCase() || 'employee'`). Siapa pun dapat mendaftar dengan `role="admin"` dan mendapatkan hak akses penuh tanpa otorisasi apa pun. Ditemukan: 11 Juli 2026. Status: **DITUNDA** bersama SEC-02 per keputusan pemilik proyek. | Ditunda (Diterima secara sadar) |
| **SEC-05** | Node 18 EOL (End-of-Life) | **Sedang (Medium)** | Dockerfile backend ([apps/backend/Dockerfile:2](file:///opt/portal-sdmv3/apps/backend/Dockerfile#L2)) & frontend ([apps/frontend/Dockerfile:2](file:///opt/portal-sdmv3/apps/frontend/Dockerfile#L2)) menggunakan `node:18-alpine`. Node 18 telah EOL sejak April 2025. Rencana: Upgrade ke Node 20 LTS / Node 22 LTS. | Terencana |
| **DATA-01** | Non-Idempotent Migrations / Data Loss Risk | **Tinggi (High)** | Skrip migrasi [migrate.ts](file:///opt/portal-sdmv3/apps/backend/db/migrate.ts) tidak melacak status migrasi lewat tabel `_migrations` dan mengeksekusi ulang seluruh `.sql` mentah secara langsung. Hal ini memicu kegagalan crash jika dijalankan ulang, sehingga operasional terpaksa melakukan reset total (`npm run reset`) yang merusak data produksi. Rencana: Ubah skrip migrasi agar mencatat riwayat ke tabel `_migrations` dan hanya menjalankan migrasi baru. | Terencana |
...
* **Privilege Escalation via Self-Registration (SEC-04 - KRITIS - DITUNDA SECARA SADAR)**:
  Endpoint `POST /api/auth/register` menerima field `role` langsung dari request body client tanpa validasi/whitelist ([auth.pengguna.service.ts:34](file:///opt/portal-sdmv3/apps/backend/src/modules/pengguna/auth.pengguna.service.ts#L34): `role: role?.toLowerCase() || 'employee'`). Siapa pun dapat mendaftar dengan `role="admin"` dan mendapatkan hak akses penuh tanpa otorisasi apa pun. Ditemukan: 11 Juli 2026. Status: **DITUNDA** bersama SEC-02 per keputusan pemilik proyek.
* **Non-Idempotent Migrations / Data Loss Risk (DATA-01 - TINGGI)**:
  Skrip migrasi [migrate.ts](file:///opt/portal-sdmv3/apps/backend/db/migrate.ts) tidak melacak status migrasi lewat tabel `_migrations` dan mengeksekusi ulang seluruh berkas `.sql` mentah. Hal ini memicu kegagalan crash jika dijalankan ulang (terdapat perintah `ALTER TABLE` tanpa check kolom), sehingga memicu data loss jika dipaksa di-reset (`npm run reset`). Ditemukan: 11 Juli 2026.

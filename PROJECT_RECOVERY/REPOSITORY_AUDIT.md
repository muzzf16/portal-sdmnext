...
| **SEC-03** | Node 18 EOL (End-of-Life) | **Sedang (Medium)** | Dockerfile backend ([apps/backend/Dockerfile:2](file:///opt/portal-sdmv3/apps/backend/Dockerfile#L2)) & frontend ([apps/frontend/Dockerfile:2](file:///opt/portal-sdmv3/apps/frontend/Dockerfile#L2)) menggunakan `node:18-alpine`. Node 18 telah EOL sejak April 2025. Rencana: Upgrade ke Node 20 LTS / Node 22 LTS. | Terencana |
| **SEC-04** | Kebocoran Kunci Rahasia / Secrets Leak | **Tinggi (High)** | Berkas `.env` (berisi key sensitif seperti `WA_API_KEY` dan `JWT_SECRET` dev) serta `.sqlite` ter-track Git sejak *initial commit* (27 Okt 2025). | Diatasi (Smoke Test) |
